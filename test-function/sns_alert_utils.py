...
def send_file_transfer_sns_alert(
    ...
    # Fix Box files display: show "NO FILES" if empty or None
    box_display = ', '.join(box_files) if box_files else "NO FILES"
    s3_display = ', '.join(s3_files) if s3_files else "NO FILES"

    # For FTP, show host name if provided, else list files or NO FILES
    if ftp_host:
        ftp_display = ftp_host
    else:
        ftp_display = ', '.join(ftp_files) if ftp_files else "NO FILES"
...
Transferred to FTP: {ftp_display}

"""

    # Removed Checksums section as requested
